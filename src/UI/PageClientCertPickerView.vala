public class Starfish.UI.PageClientCertPickerView : Gtk.Grid, ResponseView {
    ...
    public void display (Core.Response response) {
        ...
        var heading = new Gtk.Label (_("Client certificate required")) {
        ...
        var desc = new Gtk.Label (_("   The page at %s requires you to use a client certificate. This is usually reuired in order for the pod to show you some personalized content. For example by using the same certificate as last time you accessed this pod you may be able to pick up where you left off or your saved prefferences may be applied. Alternativelly, if you want to present a new identity to the server you can generate and use a new client certifcate.").printf (uri.to_string ())) {
            halign = Gtk.Align.START,
            wrap = true,
            selectable = true,
        };
        ...
        foreach (var cert_name in repo.existing_certificate_names ()) {
            var button = new Gtk.Button.with_label (cert_name) {
            ...
            attach_next_to (button, previous, Gtk.PositionType.BOTTOM, 2, 1);
            previous = button;
        }

        var create_button = new Gtk.Button.with_label (_("Create new certificate")) {
            halign = Gtk.Align.START
        };
    ...
    private void create_cert_for (Core.Uri uri) {
        try {
            var header = new Granite.HeaderLabel (_("Create certificate"));
            var desc = new Gtk.Label (_("Pick a name for the new certificate."));
                    ...
                    manager.create_client_cert.begin (name, (obj, res) => {
                        try {
                            ...
                            var err_dialog = new Granite.MessageDialog.with_image_from_icon_name (
                                 _("Failed to crete certificate"),
                                 _("Failed to create client certificate. To proceed you can retry creating the certificate, or pick an existing one."),
                                 "dialog-error",
